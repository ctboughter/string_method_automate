...
for i in range(0,len(colvar[:,2])):
        if colvar[i,8] < 0:
            colvar[i,8]=colvar[i,8]+360
        if colvar[i,7] < 0:
            colvar[i,7]=colvar[i,7]+360
        if colvar[i,3] < 0:
...
Ravg=np.average(colvar[500:end,8])
Theta_avg=np.average(colvar[500:end,3])
Phi_avg=np.average(colvar[500:end,4])
Psi_avg=np.average(colvar[500:end,5])
theta_l_avg=np.average(colvar[500:end,6])
phi_l_avg=np.average(colvar[500:end,7])
Angavg=np.average(colvar[500:end,1])
Ang2avg=np.average(colvar[500:end,2])
...
Rstd=np.std(colvar[500:end,8])
Theta_std=np.std(colvar[500:end,3])
Phi_std=np.std(colvar[500:end,4])
Psi_std=np.std(colvar[500:end,5])
theta_l_std=np.std(colvar[500:end,6])
phi_l_std=np.std(colvar[500:end,7])
Angstd=np.std(colvar[500:end,1])
Ang2std=np.std(colvar[500:end,2])
...
trj=md.load_frame('input.'+inp+'.dcd',100,top='vac_protein2.pdb')
top=trj.topology
...
final=[theta_new,phi_new,psi_new,theta_l_new,phi_l_new,R_new,pin_new]
distt=[MinDist]
# print 5 digits after the decimal place
np.savetxt('inp'+out+'.out',final,fmt='%.5e')
np.savetxt('MinDist'+inp+'.out',distt,fmt='%.4e')

standard=[Theta_std,Phi_std,Psi_std,theta_l_std,phi_l_std,Rstd]
np.savetxt('std'+out+'.out',standard,fmt='%.5e')

average=[Theta_avg,Phi_avg,Psi_avg,theta_l_avg,phi_l_avg,Ravg]
np.savetxt('avg'+out+'.out',average,fmt='%.5e')
